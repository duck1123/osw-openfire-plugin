...
public class InboxManager {
    ...
    /**
     * Retrieves the last 20 message (which are in fact received activity
     * entries) from a user inbox as a immutable list.
     *
    ...
    public List<ActivityMessage> getMessages(String userJID) {
        ...
        Query query = em.createQuery("SELECT x FROM Messages x " +
                                     "WHERE x.recipient = ?1 " +
                                     "ORDER BY x.received DESC");
